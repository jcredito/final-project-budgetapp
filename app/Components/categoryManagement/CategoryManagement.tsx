...
export default function CategoryManagement({ userId, categories, refreshCategories }: Props) {
    const [managingCategories, setManagingCategories] = useState<boolean>(false);
    const [creatingCategory, setCreatingCategory] = useState<boolean>(false);
    const [categoriesManagement, setCategoriesManagement] = useState<Category[]>(categories);
        ...
        <div className="m-50 px-28 bg-gray-100">
            {!managingCategories && (
                    <div className='px-4 py-4 flex flex-row-reverse'>
                        <button className={styles['btn-create']} onClick={() => { setManagingCategories(true) }}>Manage Categories</button>
                    </div>
                )
            }
            {
                managingCategories && (
                    <div className='py-4'>
                        ...
                        <button onClick={() => { setManagingCategories(false) }}
                        className={styles['btn-cancel']}
                        >close</button>
                        </div>
                        {!creatingCategory && (
                            <div className='py-2'><button className={styles['btn-create']} onClick={() => { setCreatingCategory(true) }}>Create Category</button></div>
                        )
                        }
                        {
                            creatingCategory &&
                            (
                                <div>
                                    <h3>New Category</h3>
                                    <CategoryManagementCreate createCategory={createCategory} setCreatingCategory={setCreatingCategory} userId={userId} />
                                </div>
                            )
                        }
                        {
