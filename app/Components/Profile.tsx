...
export default function Profile({userId, categories, transactions, budgets} : Props) {
    const [profileCategories, setProfileCategories] = useState<Category[]>(categories);
    const [profileTransactions, setProfileTransactions] = useState<Transaction[]>(transactions);
    const [profileBudgets, setProfileBudgets] = useState<Budget[]>(budgets);
    const [profileTransactionGroups, setProfileTransactionGroups] = useState<TransactionGroup[]>(categories.map(cat => {
        return {
            category: cat,
    ...
    useEffect(() => {
        const updatedTransactions = profileTransactions.map(transaction => {
            const category = profileCategories.find( c => c.id === transaction.category.id) ?? transaction.category;
            transaction.category = category;
            return transaction;
        });
        const updatedBudgets = profileBudgets.map( budget => {
            const category = profileCategories.find( c => c.id === budget.category.id) ?? budget.category;
    ...
    useEffect(() => {
        const transactionGroups = profileCategories.map(cat => {
            return {
                category: cat,
                amount: profileTransactions.filter(t => {if (t.category.id === cat.id) return t;}).reduce((tot, tran) =>  tot + (tran.amount*1), 0)
            }
        });
