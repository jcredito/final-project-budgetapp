...
export default function BudgetManagementItem({
  budget,
  transactionGroup,
  updateBudget,
  deleteBudget,
  ...
  return !isEditingBudget ? (
    <tr key={`budget-${budget.id}-item`}>
      <td className={styles['table-cell']}>{budget.category.name}</td>
      <td className={styles['table-cell']}>
        {parseFloat(`${budget.amount}`).toFixed(2)}
      </td>
      <td className={styles['table-cell']}>
        {(transactionGroup?.amount ?? 0).toFixed(2)}
      </td>
      <td className={styles['table-cell']}>
        {(budget.amount - (transactionGroup?.amount ?? 0)).toFixed(2)}
      </td>
      <td className={styles['table-cell']}>
        <button
