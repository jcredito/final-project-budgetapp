...
export default function BudgetManagement({ userId, categories, budgets, transactionGroups, setProfileBudgets }: Props) {
    ...
    useEffect(() => {
        ...
        if (categories.filter(c => !budgets.find(b => b.category.id === c.id)).length > 0) {
            setCategory(categories.filter(c => !budgets.find(b => b.category.id === c.id))[0]?.id ?? 0);
        }
    }, [budgets]);
    return (
                        ...
                        {categories.filter(c => !budgets.find(b => b.category.id === c.id)).length > 0 ? (<form onSubmit={event => event.preventDefault()} className='flex flex-row w-full'>
                            <div className="flex flex-wrap grow">
                                <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                    ...
                                    >
                                        {
                                            categories.filter(c => !budgets.find(b => b.category.id === c.id)).map(
                                                option => {
                                                    return (
                                                        <option
                                                            value={option.id}
